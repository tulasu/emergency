<script setup lang="ts">
import type { ProvisionResult } from '~/types/auth';
import type { ImportDraft } from '~/stores/import-draft';
import { emptyRow, filledUsers, replaceUsers, type UserRow } from '~/utils/users-form';
import type { UsersFileKind } from '~/utils/users-file';

definePageMeta({
  middleware: ['auth', 'staff'],
});

const { $api } = useNuxtApp();
const results = useProvisionResultStore();
const drafts = useImportDraftStore();
const { groupOptions, load, groupName } = useGroups();

const groupId = ref('');
const users = ref<UserRow[]>([emptyRow()]);
const pending = ref(false);
const error = ref('');
const fileName = ref('');
const fileKind = ref<UsersFileKind | null>(null);
const fileInput = ref<HTMLInputElement | null>(null);

onMounted(async () => {
  const draft = drafts.draft;
  if (!draft) {
    await navigateTo('/users/new', { replace: true });
    return;
  }
  applyDraft(draft);
  await load();
});

const fileMeta = computed(() => {
  const kind = fileKind.value === 'json' ? 'JSON' : 'CSV';
  return `${kind} · ${ruCount(users.value.length, 'запись', 'записи', 'записей')}`;
});

const countLabel = computed(() => {
  const base = ruCount(users.value.length, 'учётная запись', 'учётные записи', 'учётных записей');
  return fileName.value ? `${base} из ${fileName.value}` : base;
});

function applyDraft(draft: ImportDraft): void {
  fileName.value = draft.fileName;
  fileKind.value = draft.kind;
  users.value = replaceUsers(draft.users);
}

function addRow(): void {
  users.value.push(emptyRow());
}

function removeRow(index: number): void {
  users.value.splice(index, 1);
  if (users.value.length === 0) {
    addRow();
  }
}

async function onFile(event: Event): Promise<void> {
  const input = event.target as HTMLInputElement;
  const file = input.files?.[0];
  input.value = '';
  if (!file) {
    return;
  }
  const parsed = parseUsersFile(await file.text(), file.name);
  if (!parsed.ok) {
    error.value = parsed.error;
    return;
  }
  error.value = '';
  const draft = { fileName: file.name, kind: parsed.kind, users: parsed.users };
  drafts.set(draft);
  applyDraft(draft);
}

async function submit(): Promise<void> {
  const filled = filledUsers(users.value);
  if (!groupId.value || filled.length === 0 || pending.value) {
    error.value = 'Загрузите файл, выберите группу и оставьте хотя бы одну учётную запись';
    return;
  }
  pending.value = true;
  error.value = '';
  try {
    const result = await $api<ProvisionResult>('/auth/users/batch', {
      method: 'POST',
      body: { group_id: groupId.value, users: filled },
    });
    results.set(result, groupName(groupId.value));
    await navigateTo('/users/credentials');
  } catch (err) {
    error.value = apiErrorMessage(err);
  } finally {
    pending.value = false;
  }
}
</script>

<template>
  <section class="page">
    <div class="sheet">
      <header class="sheet__header">
        <div class="sheet__titles">
          <h1 class="page-title">Импорт из файла</h1>
          <p class="page-sub">Учётные записи будут добавлены в выбранную группу.</p>
        </div>
        <div class="sheet__actions">
          <TbField class="sheet__group" label="Группа">
            <TbSelect v-model="groupId" placeholder="Выберите группу" :options="groupOptions" />
          </TbField>
        </div>
      </header>

      <input
        ref="fileInput"
        class="file-input"
        type="file"
        accept=".csv,.json,text/csv,application/json"
        @change="onFile"
      />

      <div v-if="fileName" class="filebar">
        <div class="filebar__left">
          <TbIcon name="file" />
          <div class="filebar__text">
            <p class="filebar__name">{{ fileName }}</p>
            <p class="filebar__meta">{{ fileMeta }}</p>
          </div>
        </div>
        <TbButton variant="ghost" type="button" @click="fileInput?.click()">Заменить файл</TbButton>
      </div>

      <p v-if="error && !fileName" class="sheet__error">{{ error }}</p>

      <form v-if="fileName" @submit.prevent="submit">
        <UsersTable :users="users" @add="addRow" @remove="removeRow" />
        <footer class="foot">
          <p>{{ countLabel }}</p>
          <p v-if="error" class="foot__error">{{ error }}</p>
          <TbButton type="submit" :disabled="pending">Создать учётные записи</TbButton>
        </footer>
      </form>
    </div>
  </section>
</template>

<style scoped src="~/assets/css/provision-sheet.css"></style>
